...
//1. typeResult 변수 생성
//2. Boolean 내장함수 활용 data1 변수에 들어간 논리데이터종류 확인
//3. typeResult 대입
let typeResult = Boolean(data1); 
console.log(`data1의 데이터타입은 : ${typeResult}`); //출력

data1 = 'ABC'; //deta 재대입 'ABC' 재대입
...
data3 = null; //null값을 삭제할때 대입하는 키워드
typeResult = Boolean(data3);
console.log(`data3의 데이터타입은 : ${typeResult}`); //출력

deta2 = '가나다라';
deta3 = data2 * 10; //NaN(Not a Number)
console.log(data3);
...
typeResult = Boolean(data2);
console.log(`data2의 데이터타입은 : ${typeResult}`); //출력
typeResult = Boolean(data3); //대입은하나씩만
console.log(`data3의 데이터타입은 : ${typeResult}`); //출력

// 조건문
// if(조건식) {조건식이 참일 때 실행되는 실행문}
// if(조건식) {조건식이 참일 때} else{조건식이 거짓일 때 실행}

let x = 10;
let y = 20;
let result = x == y; //=(대입) ==(피연산자 같은가 비교연산)
//let result = x != y; // !==(다른가)
console.log(`result => ${result}`);

if(result) console.log('result는 참'); //실행이 한 줄이면 중괄호 생략가능
...
adBtn.addEventListener('click',function(){
    ...
    else alert('성인이 아니면 접속이 불가능 합니다.');
    // if(user){location.href = '주소'; console.log(''); //2줄 이상일때 중괄호 처리}else{alert('메세지');
});
