// function.js
// function 함수명(전달인자:필수아님){반복실행내용;}

//반복 함수 준비
function story1(){ //사용자정의함수
    story2(); //콜백함수 
    let a = '조약돌 수집';
    a += '조약돌 떨어트리기'; // 기존이야기에 더하기 +=
    a += '조약돌 따라 집 찾아오기';
    console.log(a);
}
...
//함수호출
func_z(); //NaN == Not a Number 약자 (숫자가 아니라 계산할 수 없음)
//함수준비
function func_z(){
    z = 5; // 전역변수z 값5대입 위치 = 지역스코프(전역에서 인식못함)
    func_y(); //지역변수 y10값 인식을 위해 y10을 가지고 있는 함수 호출0
    console.log(x+y); //11
}
function func_y(){
    y = 10; //전역변수y 값10대입 위치 = 지역스코프(전역에서 인신못함)
    console.log(x+y+z); //16
}

//----------------------------지역&전역 연습문제2
// 최종) A방간식:초코파이  B방간식:초코파이
...
// 지갑에 든 물건 예측하기
// 지갑 - 동전, 지갑 - 지폐
// 변수의 이름이 같아도 그 변수가 전역과 지역으로 나눠져 있다면 동일설정이 가능하다

// 변수
let wallet = '동전'; //전역변수
// 함수호출
bank(); //결과는? 지역변수 값
console.log(`wallet : ${wallet}`); //결과는? 전역변수 값
// 함수준비
function bank(){
    let wallet = '지폐'; //지역변수
...
school(); //학생3 요청
//함수준비
function school(meals='김밥'/* 매개변수 / 대입연산자(=)를 붙이면 기본값(필수X) */)
{ //매개변수는 let, const 없이도 알아서 변수 인식함
    //let meals = '부대찌개';
...
//함수호출
house(); //매개변수의 기본값이 있을 때 호출괄호는 비워도 됨
house('빵부스러기');
//함수준비
function house(material='조약돌'){
    let story=''; //지역변수
    //이야기
    story += `헨젤은 ${material}를(을) 떨어트리면서 숲으로 갔습니다`;
    story += `계모가 떠나고`;
...
coffee('아메리카노',2);
//함수준비
function coffee(menu,num=1/* 매개변수갯수 제한없음 */){
...
//----------------------------커피주문 알고리즘
// 따뜻한 카페라떼 2잔 주문완료되었습니다.
// 차가운 아메리카노 1잔 주문완료되었습니다.
// 따뜻한 녹차라떼 2잔 주문완료되었습니다.
// 차가운 아메리카노 1잔 주문완료되었습니다.

//카페메뉴배열 
const menuAll = ['카페라떼','아메리카노','녹차라떼']; //변경되지않음
console.log(menuAll);
// 함수호출
cafeKiosk('따뜻한',0,2);
cafeKiosk(undefined,1); //undefined : 정의를안하겠다(기본값으로 쓰임)
cafeKiosk('따뜻한',2,2);
