...
filterView.addEventListener('click',function(){
    ...
    if(filterListShowHide == false) {
        ...
        //위 코드 해석
        //flex 보이는 상태가 됐으므로 js의 상태변수도 참(true)로 대입해서 추적가능하게 만들기
        fiterListShowHide = !filterListShowHide;l /* ! 부정연산자 */
        //위 코드 해석
...
console.log(filterA, filterContents); //NodeList출력 시 [index] 개별구분
...
for(let f of filterA){
    ...
    f.addEventListener('click',function(e){
        ...
        for(let list of filterContents){ //필터보기메뉴 클릭 시 인식해야하는 상품목록들 
            ...
            else if(f.dataset.name == list.children[0].dataset.category) list.style.display = 'block'; //누른게 전체가 아니면
        }
    })
}